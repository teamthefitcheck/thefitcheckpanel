...
// ─── Audience Segments ───────────────────────────────────────────────────────
app.get('/admin/audience-segments', adminAuth, async (req, res) => {
  const doc = await mdb.collection('settings').findOne({}, { projection: { audience_segments: 1, _id: 0 } });
  res.json(doc?.audience_segments || []);
});

app.post('/admin/audience-segments', adminAuth, async (req, res) => {
  try {
    const { segments } = req.body || {};
    await mdb.collection('settings').updateOne({}, { $set: { audience_segments: segments, updated_at: new Date() } }, { upsert: true });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
